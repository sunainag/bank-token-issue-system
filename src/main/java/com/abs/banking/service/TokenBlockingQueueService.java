...
import com.abs.banking.model.TokenServiceMapping.ServicePriority;
...
public class TokenBlockingQueueService implements TokenQueueService {
	...
	/*
	 * Issued token is added to the counter queue Initialize the queue, if not done
	 * before
	 */
	@Override
	...
	/*
	 * The counter requests for a token from the respective counter queue. Reduce
	 * the queue size, once the token is removed from it.
	 */
	@Override
	@Transactional
	...
	public Token addToNextQueue(Token token) {
		getCurrentTokenServiceMapping(token).setPriority(ServicePriority.URGENT);
		tokenQueue.add(token);
		return token;
	}
