...
public class BankOperationsExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<Object> handleAvailCounters(CounterNotAvailableException ex, WebRequest request) {
		return handleExceptionInternal(ex, COUNTERS_UNAVAILABLE, new HttpHeaders(), HttpStatus.INTERNAL_SERVER_ERROR, request);
	}
	...
	public ResponseEntity<Object> handleTokenQueue(InvalidTokenQueueException ex, WebRequest request) {
		return handleExceptionInternal(ex, INVALID_COUNTER_TOKEN_MAP, new HttpHeaders(), HttpStatus.BAD_REQUEST, request);
	}
